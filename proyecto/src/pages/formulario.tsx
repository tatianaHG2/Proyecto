...
interface CardFormProps {
  onSubmit: (cardData: Card, id?: number) => void | Promise<any>;
  iscreating?: boolean;
  isEditing?: boolean;
}
...
const CardPreview = ({ card }: { card: Card }) => {
  ...
  return (
    <div className={`relative w-72 rounded-xl overflow-hidden  ${getTipoColor(card.Tipo)} border-2 border-red-600 shadow-[0_0_20px_rgba(139,0,0,0.3)] group`}>
      {card.Rareza && (
        <span className={`absolute top-3 right-3 z-20 ${getRarezaColor(card.Rareza)} text-white px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider shadow-lg border border-white/30 transform rotate-2`}>
          {card.Rareza}
        </span>
      )}
      <div className="absolute top-3 left-3 z-20 text-4xl font-black text-white/20 select-none">#</div>
      ...
      <div className="relative m-4 mb-2">
        ...
        <div className="relative z-10 p-1.5 from-amber-800 to-amber-600 rounded-lg shadow-xl">
          ...
          {card.Imagen ? (
            <img src={card.Imagen} alt={card.Nombre || "Vista previa"} className="relative z-10 w-full h-40 object-contain rounded-md bg-black/40"/>
          ) : (
      ...
      <div className="p-4 pt-2">
        <h3 className="text-white font-['Black_Ops_One',cursive] text-xl mb-1 text-center drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)] tracking-wider">
        ...
        {card.Descripcion && (
          <div className="mt-3 p-2 bg-black/40 rounded-lg border border-red-700/30">
            <p className="text-gray-300 text-xs italic line-clamp-2">"{card.Descripcion}"</p>
          </div>
        )}
        {card.Debilidad && (
...
const CardForm = ({ onSubmit, iscreating = false, isEditing = false }: CardFormProps) => {
  const location = useLocation();
  const navigate = useNavigate();
  const { id } = useParams();
  
  const getInitialCardState = (): Card => ({
    Nombre: "",
    Tipo: "",
    Ataque: 0,
    Defensa: 0,
  ...
  useEffect(() => {
    if (isEditing && location.state) {
      setCard(location.state as Card);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditing && id) {
      await onSubmit(card, parseInt(id));
    } else {
      await onSubmit(card);
    }
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-white text-xl font-bold">
            {isEditing ? "Actualizando carta..." : "Creando carta..."}
          </p>
        </div>
      </div>
    );
    ...
    <div className="flex flex-col lg:flex-row gap-8 justify-center items-start p-4">
      <form onSubmit={handleSubmit} className="w-full lg:w-[500px] p-6 bg-gradient-to-r from-black/90 via-red-950/90 to-black/90 border-2 border-red-700 rounded-2xl shadow-[0_0_30px_rgba(139,0,0,0.3)] backdrop-blur-sm relative overflow-hidden">
        ...
        {isEditing && (
          <div className="mb-4 p-3 bg-red-900/20 rounded-lg border border-red-700">
            <p className="text-gray-300 text-sm">
              <span className="font-semibold text-red-400">Número de carta:</span> {card.Numero || id}
            </p>
          </div>
        )}
        ...
        <div className="flex flex-col gap-4 relative z-10">
          <div>
            <label className="text-gray-300 font-semibold uppercase text-sm tracking-wider mb-2 block">Nombre</label>
            <input 
              value={card.Nombre} 
              onChange={(e) => handleChange("Nombre", e.target.value)} 
              placeholder="Ej. Mia Colucci" 
              className="w-full p-3 bg-black/80 border border-red-700 rounded-lg text-white focus:border-yellow-600 focus:ring-2 focus:ring-red-700/50 focus:scale-[1.02] transition-all duration-300 placeholder:text-gray-500"
            />
            {errors.Nombre && <span className="text-red-500 text-sm mt-1 block border-l-2 border-red-700 pl-2">{errors.Nombre}</span>}
          </div>

          <div>
            <label className="text-gray-300 font-semibold uppercase text-sm tracking-wider mb-2 block">Tipo</label>
            <input 
              value={card.Tipo} 
              ...
              className="w-full p-3 bg-black/80 border border-red-700 rounded-lg text-white focus:border-yellow-600 focus:ring-2 focus:ring-red-700/50 focus:scale-[1.02] transition-all duration-300 placeholder:text-gray-500"
            />
            {errors.Tipo && <span className="text-red-500 text-sm mt-1 block border-l-2 border-red-700 pl-2">{errors.Tipo}</span>}
          </div>
          
          <div>
            <label className="text-gray-300 font-semibold uppercase text-sm tracking-wider mb-2 block">Vida</label>
            <input 
              type="number"
              min={0}
              value={card.vida} 
              onChange={(e) => handleChange("vida", Number(e.target.value))} 
              className="w-full p-3 bg-black/80 border border-red-700 rounded-lg text-white focus:border-yellow-600 focus:ring-2 focus:ring-red-700/50 focus:scale-[1.02] transition-all duration-300"
            />
          </div>
          
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="text-gray-300 font-semibold uppercase text-sm tracking-wider mb-2 block">Ataque</label>
              <input 
                type="number" 
                min={0} 
                value={card.Ataque} 
                ...
                className="w-full p-3 bg-black/80 border border-red-700 rounded-lg text-white focus:border-yellow-600 focus:ring-2 focus:ring-red-700/50 focus:scale-[1.02] transition-all duration-300"
              />
              {errors.Ataque && <span className="text-red-500 text-sm mt-1 block border-l-2 border-red-700 pl-2">{errors.Ataque}</span>}
            </div>
            <div className="flex-1">
              <label className="text-gray-300 font-semibold uppercase text-sm tracking-wider mb-2 block">Defensa</label>
              <input 
                type="number" 
                min={0} 
                value={card.Defensa} 
                ...
                className="w-full p-3 bg-black/80 border border-red-700 rounded-lg text-white focus:border-yellow-600 focus:ring-2 focus:ring-red-700/50 focus:scale-[1.02] transition-all duration-300"
              />
              {errors.Defensa && <span className="text-red-500 text-sm mt-1 block border-l-2 border-red-700 pl-2">{errors.Defensa}</span>}
            </div>
          </div>

          <div>
            <label className="text-gray-300 font-semibold uppercase text-sm tracking-wider mb-2 block">Descripción</label>
            <textarea 
              value={card.Descripcion} 
              onChange={(e) => handleChange("Descripcion", e.target.value)} 
              placeholder="Descripción (mínimo 10 caracteres)" 
              className="w-full p-3 bg-black/80 border border-red-700 rounded-lg text-white focus:border-yellow-600 focus:ring-2 focus:ring-red-700/50 focus:scale-[1.02] transition-all duration-300 placeholder:text-gray-500 resize-none min-h-[100px]"
            />
            {errors.Descripcion && <span className="text-red-500 text-sm mt-1 block border-l-2 border-red-700 pl-2">{errors.Descripcion}</span>}
          </div>
          
          <div>
            <label className="text-gray-300 font-semibold uppercase text-sm tracking-wider mb-2 block">Debilidad</label>
            <input 
          ...
          <div>
            <label className="text-gray-300 font-semibold uppercase text-sm tracking-wider mb-2 block">Rareza</label>
            <input 
              value={card.Rareza} 
          ...
          <div>
            <label className="text-gray-300 font-semibold uppercase text-sm tracking-wider mb-2 block">Imagen (URL)</label>
            <input 
              value={card.Imagen} 
      ...
      <div className="w-full lg:w-auto flex flex-col items-center">
        <div className="mb-4 text-center">
          <h3 className="text-white font-['Black_Ops_One',cursive] text-xl border-b-2 border-red-700 pb-2 inline-block px-6">VISTA PREVIA</h3>
        </div>
        <CardPreview card={card} />
        <p className="text-gray-400 text-sm mt-4 text-center max-w-xs">Vista en tiempo real de cómo se verá tu carta</p>
      </div>
    </div>
  );
};
